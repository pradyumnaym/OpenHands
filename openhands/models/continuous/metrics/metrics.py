...
def get_wer(
    ...
    reference_gloss_sequences,
    gloss_vocab
):
    decoded_hyp = [gloss_vocab.lookup_tokens(x) for x in decoded_gloss_sequences]
    decoded_ref = [gloss_vocab.lookup_tokens(x) for x in reference_gloss_sequences]

    gls_ref = [cleanup_function(" ".join(t)) for t in decoded_ref]
    gls_hyp = [cleanup_function(" ".join(t)) for t in decoded_hyp]
...
def get_bleu(
    ...
    metrics_dict['rouge'] = rouge(references=txt_ref, hypotheses=txt_hyp)
