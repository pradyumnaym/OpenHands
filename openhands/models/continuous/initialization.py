...
def initialize_model(model: nn.Module, config: dict, txt_padding_idx: int) -> None:
    """
    ...
    gain = float(config.get("init_gain", 1.0))  # for xavier
    init = config.get("initializer", "xavier")
    init_weight = float(config.get("init_weight", 0.01))

    embed_init = config.get("embed_initializer", "normal")
    embed_init_weight = float(config.get("embed_init_weight", 0.01))
    embed_gain = float(config.get("embed_init_gain", 1.0))  # for xavier

    bias_init = config.get("bias_initializer", "zeros")
    bias_init_weight = float(config.get("bias_init_weight", 0.01))
    ...
    with torch.no_grad():
        ...
        # zero out paddings
        if model.decoder.embedding is not None:
            model.decoder.embedding.lut.weight.data[txt_padding_idx].zero_()

        orthogonal = config.get("init_rnn_orthogonal", False)
        lstm_forget_gate = config.get("lstm_forget_gate", 1.0)

        # encoder rnn orthogonal initialization & LSTM forget gate
        if hasattr(model.encoder_seq[-1], "rnn"):
